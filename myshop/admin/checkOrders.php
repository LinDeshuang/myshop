	...
	<section>
		...
		<?php
		    ...
		    if($rows>0)
		    {
		       ...
		       echo "<h2>未审核订单信息如下</h2>";
		       outputInfo($rows,$page,$pageSize);
		       echo "<form method='post' action='checkOrders.php'>";
	           echo "<table class='table table-hover table-bordered order-list'>";
	           echo "<tr><th>订单号</th><th>用户名</th><th>商品</th><th>配送方式</th><th>支付方式</th><th>收货地址</th><th>下单时间</th><th>总价</th><th>订单状态</th><th>审核</th></tr>";
	           foreach ($pageRecords as $value) 
	           {
	        	...
	        	echo "<td>
                      <input id='check1{$orderID}' name='check{$orderID}' type='radio' value='审核通过' required/>
                      <label for='check1{$orderID}' class='btn btn-success'>审核通过</label><br/>
	        	      <input id='check2{$orderID}' name='check{$orderID}' type='radio' value='审核失败' required/>
	        	      <label for='check2{$orderID}' class='btn btn-warning'>审核失败</label>
	        	      </td>";
	        	echo "</tr>";          
	           }
	           echo "</table>";
			  ...
			  if($rows>$pageSize)
			  {
				  ...
				  $target="checkOrders.php";
				  outputNav($pages,$page,$target);
			  }
		    }
	        else
	        {
	        	echo "<p class='info'>目前没有未审核的订单！</p>";
	        }
		?>
	</section>
</div>
